...
extern UINT8    usb_rx_endIndex;
extern UINT32   usb_rec_number;
extern UINT8    vcom_buf_ID;
extern UINT8    vcom_out_index;
//extern vcom_rep vcom_rp[2];
extern UINT8    vcom_buf[2][1024*16];   //1024*8
extern UINT8    gu8EP3_OUT_Buffer[16384+512];
extern UINT32   PIT1_USB_Val,PIT1_Uart_Val;
extern void PIT1_reload(UINT32 val);
...
void USB_CDC_ISR(void)
{
	...
	if ((ucISR & (1<<INDEX_EP2)))
	{
		...
		recvLen = usb_cdc_receive(INDEX_EP2,usb_rx_buf);
//		
		
#if (UARTBUS_OPTION == 4)
		for(i=0;i<recvLen;i++) 
		{
			Hid_Rx_Buffer_long[Hid_Rx_Count_long++ % Hid_SIZE] = usb_rx_buf[i];
			Hid_Rx_Num ++;
		}
#endif
//			
...
//		else
//		{	
//			if((gu8EP3_OUT_Buffer[0]==0x4A)||(gu8EP3_OUT_Buffer[0]==0x6C))
//			{
//				uart_act_index = 0;
//				PIT1_reload(PIT1_USB_Val);
//			}
//			else  
//			{
//				PIT1->PCSR &= (~PCSR_EN);
